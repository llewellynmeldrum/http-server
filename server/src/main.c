...
int main(int argc, char **argv) {
  ...
  while (accepting_connections) {
    ...
    *client =
        accept(config.server_fd, (sockaddr *)&client_addr, &client_addr_len);
    if (*client == SOCK_ERR) {
...
void init_socket() {
  ...
  if (config.server_fd == SOCK_ERR)
    logfatal_exit("Unable to create socket.\n");
  ...
  if (listen_status == SOCK_ERR)
    logfatal_exit("Unable to start listening for connections.\n");
}
